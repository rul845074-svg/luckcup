...
| 后端部署 | 腾讯云 SCF（Serverless Cloud Function） | 通过 API 网关暴露 HTTP 接口 |
| 前端 API 地址 | 构建时必须显式设置 `VITE_API_URL=SCF的API网关地址` | 禁止依赖 `'/api'` fallback，那只在本地 vite dev 代理时有效 |
...
# 前端构建（必须带环境变量）
cd luckcup-web
VITE_API_URL=你的SCF网关地址 npm run build
# 产物在 dist/ 目录，上传到腾讯云 COS
...
1. **先看文档再动手** — 项目的学习文档在 `学习/` 目录，VERSION.md 和需求/架构文档都在里面，改代码前先读
