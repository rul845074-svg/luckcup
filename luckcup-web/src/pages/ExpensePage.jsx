...
import { C, CATEGORY_ICONS } from '../constants/theme';
...
export default function ExpensePage() {
  ...
  const loadExpenses = () => {
    expensesApi.getList(currentMonth()).then(data => setExpenses(data.expenses));
  };

  useEffect(() => {
    settingsApi.get().then(s => setCategories(s.expenseCategories));
    loadExpenses();
  }, []);

  const handleSave = async () => {
    if (!selected) { showToast('请选择支出类别'); return; }
    if (!amount || Number(amount) <= 0) { showToast('请输入有效金额'); return; }
    if (!note.trim()) { showToast('备注不能为空'); return; }

    setSaving(true);
    try {
      if (editItem) {
        await expensesApi.update(editItem.id, { date, category: selected, amount: Number(amount), note });
        showToast('修改成功 ✓');
        setEditItem(null);
      } else {
        await expensesApi.add({ date, category: selected, amount: Number(amount), note });
        showToast('保存成功 ✓');
      }
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      <PageHeader title={editItem ? '编辑支出' : '记支出'} subtitle="发生一笔，记一笔" />
      ...
      <div style={{ padding: '16px 16px 0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            {categories.map(c => (
              <button key={c} onClick={() => setSelected(c)} style={{
                ...
                <span style={{ fontSize: 24 }}>{CATEGORY_ICONS[c] || '📋'}</span>
                <span style={{
                  fontSize: 13,
        ...
        <Card style={{ marginTop: 12 }}>
          ...
          <p style={{ fontSize: 13, color: C.textSec, margin: '0 0 8px' }}>备注（必填）</p>
          <textarea value={note} onChange={e => setNote(e.target.value)}
            placeholder="记录这笔支出的用途..." style={{
            ...
            <div key={e.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>
                  {e.category} {e.is_auto && <span style={{ fontSize: 11, color: C.textSec }}>(自动)</span>}
                </div>
